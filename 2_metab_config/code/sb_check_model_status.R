...
sb_check_model_status <- function(metab.file, phase=c('stage','post'), smu.config) {
  ...
  if(phase=='stage') {
    metab.table$local <- ifelse(is.na(metab.table$local), NA, file.exists(metab.table$filepath))
    needed <- dplyr::filter(metab.table, !local, !posted, !tagged) 
    ...
    needed <- filter(metab.table, local & (!posted | !tagged))
  }
