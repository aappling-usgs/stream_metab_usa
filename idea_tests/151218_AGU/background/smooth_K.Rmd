...
data_choices <- c(local.time="sitetime_calcLon", DO.obs="doobs_nwis", DO.sat="dosat_calcGGbts", 
...
data <- ts_merged %>% setNames(c("DateTime", names(data_choices))) %>% select(-DateTime) %>% unitted::v() %>%
  filter(local.time > as.POSIXct("2013-01-01") & local.time < as.POSIXct("2015-01-01"))
...
# smooth the K600s
tdata_choices <- c(local.time="sitetime_calcLon", discharge='veloc_calcDischHarvey')
