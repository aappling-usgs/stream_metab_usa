...
true_ests <- bind_rows(lapply(1:3, function(iter) {
  ...
  fits <- lapply(1:nrow(params), function(ip) {
    ...
    metab_mle(data=all_data, calc_DO_fun=calc_DO_mod_by_step)
  })
  
  true_est <- bind_rows(lapply(1:length(fits), function(fitnum) {
    p <- params[fitnum,c("GPP","ER","k","stream","error","oesd","pesd","pephi")]
    fit <- fits[[fitnum]]
    est <- setNames(predict_metab(fit)[2,c("GPP","ER","K600")], c("GPP","ER","k"))
...
ggplot(oneeach, aes(x=true.k, y=est.k, color=true.error, shape=true.stream)) + geom_abline(color="grey") + geom_point(size=2, alpha=0.9, position=position_jitter(width=.2, height=0)) + theme_bw() + scale_shape_discrete(solid=FALSE)
...
ggplot(obserrs, aes(x=true.oesd, y=est.k-true.k, color=true.stream)) + geom_point(size=2, alpha=0.9, position=position_jitter(width=.005, height=0)) + theme_bw() + scale_shape_discrete(solid=FALSE) + facet_wrap(~true.stream, scales="free_y") + scale_color_discrete(guide=FALSE)
ggplot(obserrs, aes(x=true.oesd, y=est.ER-true.ER, color=true.stream)) + geom_point(size=2, alpha=0.9, position=position_jitter(width=.005, height=0)) + theme_bw() + scale_shape_discrete(solid=FALSE) + facet_wrap(~true.stream, scales="free_y") + scale_color_discrete(guide=FALSE)
ggplot(obserrs, aes(x=true.oesd, y=est.GPP-true.GPP, color=true.stream)) + geom_point(size=2, alpha=0.9, position=position_jitter(width=.005, height=0)) + theme_bw() + scale_shape_discrete(solid=FALSE) + facet_wrap(~true.stream, scales="free_y") + scale_color_discrete(guide=FALSE)

# variation in proc error
procerrs <- filter(true_ests, true.oesd==0 & true.pephi==0)
ggplot(procerrs, aes(x=true.pesd, y=est.k-true.k, color=true.stream)) + geom_point(size=2, alpha=0.9, position=position_jitter(width=.0005, height=0)) + theme_bw() + scale_shape_discrete(solid=FALSE) + facet_wrap(~true.stream, scales="free_y") + scale_color_discrete(guide=FALSE)
ggplot(procerrs, aes(x=true.pesd, y=est.ER-true.ER, color=true.stream)) + geom_point(size=2, alpha=0.9, position=position_jitter(width=.0005, height=0)) + theme_bw() + scale_shape_discrete(solid=FALSE) + facet_wrap(~true.stream, scales="free_y") + scale_color_discrete(guide=FALSE)
ggplot(procerrs, aes(x=true.pesd, y=est.GPP-true.GPP, color=true.stream)) + geom_point(size=2, alpha=0.9, position=position_jitter(width=.0005, height=0)) + theme_bw() + scale_shape_discrete(solid=FALSE) + facet_wrap(~true.stream, scales="free_y") + scale_color_discrete(guide=FALSE)

# variation in proc error
aperrs <- filter(true_ests, substr(true.error, 1, 4) %in% c("none","auto"))
ggplot(aperrs, aes(x=true.pephi, y=est.k-true.k, color=true.stream)) + geom_point(size=2, alpha=0.9, position=position_jitter(width=.0005, height=0)) + theme_bw() + scale_shape_discrete(solid=FALSE) + facet_wrap(~true.stream, scales="free_y") + scale_color_discrete(guide=FALSE)
ggplot(aperrs, aes(x=true.pephi, y=est.ER-true.ER, color=true.stream)) + geom_point(size=2, alpha=0.9, position=position_jitter(width=.0005, height=0)) + theme_bw() + scale_shape_discrete(solid=FALSE) + facet_wrap(~true.stream, scales="free_y") + scale_color_discrete(guide=FALSE)
ggplot(aperrs, aes(x=true.pephi, y=est.GPP-true.GPP, color=true.stream)) + geom_point(size=2, alpha=0.9, position=position_jitter(width=.0005, height=0)) + theme_bw() + scale_shape_discrete(solid=FALSE) + facet_wrap(~true.stream, scales="free_y") + scale_color_discrete(guide=FALSE)
aperrs %>% group_by(true.stream, true.error) %>% summarize(
  ...
  k.err=mean(est.k-true.k), k.err.sd=sd(est.k-true.k))
