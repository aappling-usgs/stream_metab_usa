...
* *Watershed Area*: metabolism sites have slightly larger watershed areas than all NHD+ reaches
...
Below is a map showing the geographic location of the metabolism sites (red dots) and a count of the metabolism sites located in each of the level III ecoregions of the contiguous US (color of ecoregion based on number of sites).  
...
```
```{r ecoregion_table, echo=FALSE, warning=FALSE, message=FALSE, eval=TRUE, results="as.is"} 
...
datatable(eco.table[srt.i, ], rownames = FALSE)
```
